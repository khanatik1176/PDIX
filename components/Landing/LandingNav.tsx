...
const LandingNav: FC<LandingGeneralProps> = ({
  isDarkModeActive,
}) => {
  ...
  return (
    <nav className='sticky top-0 z-50 px-6 pt-6 lg:px-36 2xl:px-[384px]'>
      <div
        className={` ${isDarkModeActive ? 'flex min-h-20 w-full items-center justify-between rounded-xl bg-[#3D3D3D] shadow-sm' : 'flex min-h-20 w-full items-center justify-between rounded-xl bg-white shadow-md'}`}
      >
        ...
        <ul className='hidden items-center justify-end gap-5 pr-10 lg:flex'>
          {/* <Link href={'/about'}>
            <li
          ...
          <Link href={'/casestudies'}>
            <li
              className={`flex cursor-pointer items-center justify-center gap-1 ${isDarkModeActive ? 'text-xl font-semibold text-white' : 'text-xl font-semibold text-primary'}`}
            >
              Case studies
            </li>
          </Link>
          <li>
            <div className='rounded-full bg-blue-500 p-2 lg:bg-transparent'>
              {isDarkModeActive ? (
                <Moon
                  size={24}
                  className='cursor-pointer text-white lg:text-white'
                  strokeWidth={1.5}
                  onClick={toggleMode}
                />
              ) : (
      ...
      <AnimatePresence>
        {isSheetOpen && (
          ...
          >
            ...
            <Link href={'/'}>
              <div className='absolute bottom-12 left-1/2 -translate-x-1/2 transform' onClick={toggleSheet}>
                <Image
                  src={isDarkModeActive ? DarkNavLogo : NavLogo}
                  width={180}
                  height={24}
